#!/usr/bin/env zsh
# ziro req - Gather and refine requirements
...
ziro_req_create() {
    ...
    [[ -z "$feature_name" ]] && echo "❌ Feature name required: ziro feature create [name]" && return 1

    mkdir -p "$feature_dir"

    local requirements_file="$feature_dir/requirements.md"
    ziro_check_approval "$requirements_file" "Requirements" "$2" || return 1
...
ziro_req_edit() {
    ...
    ziro_validate_feature "$feature_name" "feature edit" || return 1
...
ziro_req() {
    ...
    case $action in
        create) ziro_req_create "$feature_name" "$3" ;;
        edit) ziro_req_edit "$feature_name" "$3" ;;
